...
def get_similar_artists(h5,songidx=0):
    ...
    a=[]
    simi=0
    while (simi != h5.root.metadata.similar_artists.nrows):
        a.append(h5.root.metadata.similar_artists[songidx+simi])
        simi=simi+1
    return a
...
def get_all_terms(h5,songidx=0):
    """                                               
    Get artist terms array. Takes care of the proper indexing if we are in aggregate file.
    """
    c=[]
    genre=0
    while (genre != h5.root.metadata.artist_terms.nrows):
        c.append(h5.root.metadata.artist_terms[songidx+genre])
        genre=genre+1
    return c
